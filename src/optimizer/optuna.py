...
class OptunaOptimizer:
    def __init__(self, scenario: int, n_trials: int = 50,
                 ...
                 pred_len: int = 3, epochs: int = 100, patience: int = 10,
    ...
    def _make_trainer(self, params: dict):
        shared = dict(
    ...
    def _save_top3(self, study: optuna.Study) -> None:
        ...
        for rank, trial in enumerate(top3, start=1):
            trainer = self._make_trainer(trial.params)
            best_val, best_state, metrics = trainer.train()

            print(f"\n  Rank {rank} | val={trial.value:.4f} | params={trial.params}")
            ...
            lines = [
                ...
                *[f"  {k:<12}: {v}" for k, v in trial.params.items()],
                ...
                "",
                "── Test metrics ─────────────────────────────",
                f"  {'MAE':<12}: {metrics['MAE']:.4f}",
                f"  {'MSE':<12}: {metrics['MSE']:.4f}",
                f"  {'RMSE':<12}: {metrics['RMSE']:.4f}",
                f"  {'MAPE (%)':<12}: {metrics['MAPE']:.4f}",
                f"  {'TC_min':<12}: {metrics['TC_min']:.2f}",
                f"  {'c_s*':<12}: {metrics['c_s_star']:.4f}",
            ]
...
if __name__ == "__main__":
    set_seed()
    OptunaOptimizer(scenario=1, n_trials=50).run()
