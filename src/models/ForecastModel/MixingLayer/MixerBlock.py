...
class MixerBlock(nn.Module):
    def __init__(self, seq_length: int, ff_dim: int, dropout: float, num_features: int = 6):
        super().__init__()
        self.time_mixing = TimeMixingLayer(seq_length, dropout)
        self.feature_mixing = FeatureMixingLayer(ff_dim=ff_dim, dropout=dropout, num_features=num_features)
